...
% of length $g = (N-p)/p$. This is estimated, for large $g$, as
...
%
    % C: Shows the residual dependence of collapse transition energy from
    %   $\epsilon_u$ after neglecting the direct bridging beads effect
    %   by summing $\epsilon_u$ to $\epsilon_l$. The plot shows in
    %   function of $\epsilon_u$ the residual difference of the collapse
    %   point to the one belonging to the curve with $\epsilon_u = 0$. The
    %   collapse points align themself on a power law with fit 
    %   $\left. {\epsilon_l}^* \right|_{\epsilon_u = 0} -
    %   \left.{\epsilon_l}^*\right|_{\epsilon_u}  =  \epsilon_u + 4.85
    %   {\epsilon_u}^{1.68}$.}}
%%%
