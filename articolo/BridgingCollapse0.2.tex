...
\begin{figure}
  ...
  \caption{Collapse due to bridging proteins.  A: Switch-like
    transition to a collapsed state for polymers with small $p$ and
    $N=256$, solid lines show the predictions of the theory \ref{eq:}
    taking into account the correct scaling of $P_c(s) ~ s^{-2.27}$
    for swollen polymers but not the star polymer contribution (see
    Appendix).
%
B: Collapse curves (swelling parameter vs
    ...
    the case $\eta=1$ corresponds to the case of uniform
    collapse. The parameters have been chosen so that the
    ...
    collapse (eq. \ref{eq:deg_mfield}) on the parameters $B_1$ and
...
increase of $K_d$ correspond to a rescaling of the collape curve.
%
% Simulations are  
